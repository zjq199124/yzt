...
public class MsRoleController {
    ...
    @Autowired
    private IMsRoleService service;

    @Autowired
    private IHsRoleService hsRoleService;


    @ApiOperation(value = "增加角色", notes = "增加角色")
    @PostMapping("/addRole")
    public Result addRole(@RequestBody MsRole role) {
        role.setStatus(1);
    ...
    @PostMapping("/setRole")
    public Result setRole(@RequestBody MsRole role) {
    ...
    public Result getRole(Long id) {
        MsRole role = service.getRole(id);
        return Result.success(role);
    }
    ...
    @ApiOperation(value = "获取医院端系统内置角色列表", notes = "获取医院端系统内置角色列表")
    @ApiImplicitParams({})
